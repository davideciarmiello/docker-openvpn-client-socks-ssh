...
if [ -n "${SSH_SETTINGS}" ]; then
    SETTINGS=$(echo $SSH_SETTINGS | tr "," "\n")
    for U in $SETTINGS; do
        _SET=$(echo "${U}" | xargs)
        echo ">> Apply custom sshd_config setting: $_SET"
        echo "set /files/etc/ssh/sshd_config/$_SET" | augtool -s 1> /dev/null        
    done
fi
